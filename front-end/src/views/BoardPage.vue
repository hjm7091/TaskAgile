        ...
        <div class="board">
          ...
          <div class="board-body">
            <Draggable v-model="cardLists" class="list-container" @end="onCardListDragEnded"
                       handle=".list-header" animation="0" touchStartThreshold="20">
...
export default {
  ...
  methods: {
    ...
    onRealTimeUpdated (update) {
      ...
      if (update.type === 'cardPositionChanged') {
        this.onCardPositionChanged(update.cardPositions, update.positionHistory)
      }
    },
    onCardAdded (card) {
    ...
    deleteCardFromList (cardList, cardId) {
      const existingIndex = cardList.cards.findIndex(existingCard => { return existingCard.id === cardId })
      if (existingIndex !== -1) {
        return cardList.cards.splice(existingIndex, 1)[0]
      }
      return null
    },
    onCardPositionChanged (cardPositions, positionHistory) {
      const fromListId = positionHistory.fromListId
      const toListId = positionHistory.toListId
      if (fromListId === toListId) {
        this.updateValue(cardPositions)
        this.sortByPosition(fromListId)
      } else {
        const cardId = positionHistory.cardId
        const fromList = this.cardLists.filter(cardList => cardList.id === fromListId)[0]
        const toList = this.cardLists.filter(cardList => cardList.id === toListId)[0]
        const deletedCard = this.deleteCardFromList(fromList, cardId)
        if (deletedCard !== null) {
          this.appendCardToList(toList, deletedCard)
          this.updateValue(cardPositions)
          this.sortByPosition(toListId)
        }
      }
    },
    updateValue (cardPositions) {
      cardPositions.forEach(cardPosition => {
        const cardListId = cardPosition.cardListId
        const cardId = cardPosition.cardId
        const position = cardPosition.position
        const cardList = this.cardLists.filter(cardList => cardList.id === cardListId)[0]
        const card = cardList.cards.filter(card => card.id === cardId)[0]
        card.position = position
      })
    },
    sortByPosition (cardListId) {
      const cardList = this.cardLists.filter(cardList => cardList.id === cardListId)[0]
      cardList.cards.sort((card1, card2) => card1.position - card2.position)
    },
    openCard (card) {
